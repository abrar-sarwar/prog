"""Persistent top-10 leaderboard channel embed.

The leaderboard message in each guild's configured leaderboard channel is
a single text *embed* that prog keeps in sync with the current top 10.
The top 10 (ranked by total XP, desc) is formatted as one description
block by :mod:`core.leaderboard_embed`; this cog owns the
sending/editing, caching, and debounce.
...
Cache
-----
* ``_cache``: guild_id -> ordered ``(user_id, level, xp)`` tuple of the
  most recently *rendered* top N. Used to short-circuit redundant Discord
  edits when nothing displayed changed. XP is part of the key because the
  embed shows XP, so an XP change must refresh the message.
...
# Embed accent — gold, carrying the "1st place is the hero" tone from the
# design reference across the whole card.
LEADERBOARD_COLOR = discord.Color(0xFFD166)
LEADERBOARD_TITLE = "leaderboard"
LEADERBOARD_FOOTER = "Top 10 · ranked by total XP"


# Cache key: ordered ``(user_id, level, xp)`` per position. Comparison is
# order-sensitive, so a position swap within the top N counts as a change.
# XP is part of the key because the embed now *shows* XP, so an XP change
# (even without a level/order change) must refresh the displayed value.
CacheKey = tuple[tuple[int, int, int], ...]


def _make_cache_key(top_users: Sequence[User]) -> CacheKey:
    """Build the comparable cache key from a list of top-N user rows."""
    return tuple((u.user_id, u.level, u.xp) for u in top_users)
...
class LeaderboardChannel(commands.Cog):
    ...
    async def _push_update(
        ...
        """Build the embed and edit/send the persistent leaderboard message."""
        ...
        embed = self._build_embed(guild, top_users)
    ...
    def _build_embed(
        ...
        """Assemble the top-10 leaderboard embed from the ranked rows."""
        rows = [
            LeaderboardRow(
                rank=i + 1,
                user_id=u.user_id,
        ...
        embed = discord.Embed(
            ...
            description=format_leaderboard_description(rows),
            color=LEADERBOARD_COLOR,
        )
        embed.set_footer(text=LEADERBOARD_FOOTER)
        # Carry the server's identity without rendering an image.
    ...
    async def _edit_or_send(
        ...
        send a fresh one. ``attachments=[]`` on edit drops any image left
        over from the previous image-based card. Returns the resulting
        message, or None on failure.

        Mentions in the embed description never notify, but we still send
        with no allowed mentions as a belt-and-suspenders guard."""
        ...
        if message_id is not None:
            try:
                ...
                return None
        try:
            return await channel.send(embed=embed, allowed_mentions=no_pings)
        except discord.Forbidden:
            log.warning(
